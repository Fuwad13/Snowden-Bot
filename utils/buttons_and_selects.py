...
class QuickFightView(ui.View):
	...
	async def heal_button(self, button, interaction):
		...
		if self.qf_dict[str(self.active_p.id)]['hp'] + heal >= 100:
			...
			self.qf_dict[str(self.active_p.id)]['hp']+=heal
			hp = self.qf_dict[str(self.active_p.id)]['hp']
...
class Tradeview(ui.View):
	...
	def __init__(self,*, ctx, player1, player2, bfh , embed):
		...
		self.confim_dict = {
			str(player1.id) : False,
			str(player2.id) : False
		}

	async def interaction_check(self, interaction: discord.Interaction) -> bool:
		p1 = interaction.user == self.player1
		p2 = interaction.user == self.player2
		chk = p1 or p2
		if not chk :
